...
class Args:
    ...
    mi_use: bool = True
    """whether to use the MI-based intrinsic bonus"""
    mi_num_samples: int = 4
    """number of posterior samples per timestep to approximate MI"""
    mi_beta: float = 0.1
    """weight of the MI bonus added to env reward"""
...
@torch.no_grad()
def debug_mi_histograms(Model, agent, rb, args, writer=None, global_step=0):
    """
    Sample a batch from the replay buffer, compute MI bonus, and
    print / log histograms for transitions with r_{t+1} > 0, < 0, and == 0.

    This is purely a diagnostic tool.
    """
    # 1) Sample a batch of sequences
    data = rb.sample(args.batch_size)
    images   = data["obs"].to(dtype=torch.float32).div_(255.)  # (B, S, 1, H, W)
    actions  = data["action"]                                  # (B, S)
    step_ty  = data["step_type"]                               # (B, S)
    rewards  = data["reward"]                                  # (B, S)
    dones    = data["done"]                                    # (B, S), unused here

    B, S = actions.shape
    T = S - 1

    # 2) Get posterior dists and MI bonus on transitions
    ...
    # 3) Align MI with r_{t+1}
    # r_tp1[b, t] = rewards[b, t+1]  for t = 0..T-1
    r_tp1 = rewards[:, 1:]       # (B, T)

    mi_flat   = mi_bonus.reshape(-1).cpu().numpy()   # (B*T,)
    r_tp1_flat = r_tp1.reshape(-1).cpu().numpy()     # (B*T,)

    pos_mask  = r_tp1_flat > 0.0
    neg_mask  = r_tp1_flat < 0.0
    zero_mask = r_tp1_flat == 0.0

    mi_pos  = mi_flat[pos_mask]
    mi_neg  = mi_flat[neg_mask]
    mi_zero = mi_flat[zero_mask]

    # 4) Print simple stats
    print("\n[MI debug] global_step =", global_step)
    print("  total transitions:", mi_flat.shape[0])
    print("  >0 reward count:", mi_pos.shape[0])
    print("  <0 reward count:", mi_neg.shape[0])
    print("  =0 reward count:", mi_zero.shape[0])
    ...
    safe_stats(mi_pos,  "MI | r>0")
    safe_stats(mi_neg,  "MI | r<0")
    safe_stats(mi_zero, "MI | r=0")

    # 5) Optional: log histograms to TensorBoard
    if writer is not None:
        if mi_pos.size > 0:
            writer.add_histogram("debug/mi_pos", mi_pos, global_step)
        if mi_neg.size > 0:
            writer.add_histogram("debug/mi_neg", mi_neg, global_step)
        if mi_zero.size > 0:
            writer.add_histogram("debug/mi_zero", mi_zero, global_step)
        writer.add_histogram("debug/mi_all", mi_flat, global_step)
        ...
        if global_step > args.learning_starts and global_step % args.train_frequency == 0:            
            ...
            with torch.no_grad():
                ...
                if args.mi_use and args.mi_beta > 0.0:
                    # mi_bonus : (B, S-1), aligned with transitions t=0..S-2
                    mi_bonus = compute_mi_bonus(
                        agent      = agent,
                        q_z1       = q_z1,
                        q_z2       = q_z2,
                    ...
                    # Build total reward: r_total_{t+1} = r_env_{t+1} + β * I_t
                    rewards_total = rewards.clone()
                    rewards_total[:, 1:] = rewards_total[:, 1:] + args.mi_beta * mi_bonus
                else:
                    rewards_total = rewards
            
            # Standard C51 loss but on augmented rewards
            q_loss, q_pred, target_q = agent.compute_loss(
                z1, z2, actions, rewards_total, dones)
            agent.update(q_loss)
            ...
            if global_step % 500 == 0 and args.mi_use and args.mi_beta > 0:
                writer.add_scalar("charts/mi_bonus_mean", mi_bonus.mean().item(), global_step)
            ...
            if global_step % 100000 == 0 and args.mi_use and args.mi_beta > 0.0:
                debug_mi_histograms(
                    Model   = Model,
                    agent   = agent,
                    rb      = rb,
